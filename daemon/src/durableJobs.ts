...
export class DurableJobManager {
  ...
  async submit(action: DurableJobRecord["action"], params: Record<string, unknown>): Promise<ApiResponse> {
    ...
    if (action === "syncAtlasBatch") {
      ...
      const total = atlasItemCount(params);
      if (params.confirm !== true) {
        return ok({
          ...
          rootId: params.rootId,
          batchId: params.batchId,
          warning: "syncAtlasBatch is experimental and not daemon-undoable. Pass confirm:true only for a dedicated, backed-up root.",
        });
      }
      ...
      const job = createDurableJob(action, atlasSafeParams(payload), total);
      ...
      await this.save(job);
      await appendAudit(this.config.logDir, {
  ...
  private async process(job: DurableJobRecord): Promise<void> {
    ...
    try {
      ...
      await this.save(job);
      if (job.action === "syncAtlasBatch") {
        await removeAtlasPayload(this.config.appDir, job.jobId);
        await appendAudit(this.config.logDir, {
          ts: new Date().toISOString(),
          action: job.action,
          opId: job.jobId,
      ...
      if (job.action === "syncAtlasBatch") {
        await appendAudit(this.config.logDir, {
          ...
          targetIds: job.ids,
          count: job.total,
          status: job.status === "outcome_unknown" ? "outcome_unknown" : "error",
          durationMs: Date.now() - startedAt,
